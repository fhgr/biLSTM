...
TRAINING_CORPUS_X = ["html_training_corpus-g{}-x.?.bin.gz", ]
TRAINING_CORPUS_Y = ["html_training_corpus-g{}-y.?.bin.gz", ]
...
def l(corpus_pattern, gap_size):
    ''' loads all corpora matching the given corpus_pattern '''
    result = []
    for fname in sorted(glob(corpus_pattern.format(gap_size))):
        print("Opening", fname)
...
if __name__ == '__main__':
    ...
    for gap_size in (1, 2, 3):
        ...
        for training_corpus_x, training_corpus_y in zip(TRAINING_CORPUS_X, TRAINING_CORPUS_Y):
            print("Training pattern:", training_corpus_x)
            data_x = np.asarray(l(training_corpus_x, gap_size))
            data_y = to_categorical(np.asarray(l(training_corpus_y, gap_size)), num_classes=len(vocabulary))
